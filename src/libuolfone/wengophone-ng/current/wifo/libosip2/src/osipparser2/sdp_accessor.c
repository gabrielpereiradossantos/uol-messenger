...
/* <UOLFONE> */
sdp_attribute_t *
sdp_message_att_find(sdp_message_t * sdp, int pos_media, char *field, char *payload_id)
{
  sdp_attribute_t *attribute = NULL;
  int attrib_index = 0;
  
  while ((attribute = sdp_message_attribute_get(
      sdp, pos_media, attrib_index)) != NULL)
  {
    if (strcmp(attribute->a_att_field, field) == 0)
    {
      if ( (strcmp(field, "rtpmap") == 0) || 
          (strcmp(field, "fmtp") == 0) || 
          (strcmp(field, "AS") == 0) )
      {
        if (strstr(attribute->a_att_value, payload_id) == attribute->a_att_value)
        {
          return attribute;
	    }
      }
...
int
sdp_message_get_media_pos(sdp_message_t * sdp, char *media_id)
{
  sdp_media_t *media = NULL;
  int media_index = 0;
  
  while ((media = osip_list_get(&sdp->m_medias, media_index)) != NULL)
  {
    if (strcmp(media->m_media, media_id) == 0)
    {
		return media_index;
    }
