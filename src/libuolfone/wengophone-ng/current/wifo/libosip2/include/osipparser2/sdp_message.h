...
/* <UOLFONE> */
/**
 * Search the media attributes and return the one that matches 
 * the "field" value.
 * @param sdp The element to work on.
 * @param pos_media The media line number.
 * @param field The attribute field to search for. This value must match.
 * @param payload_id The payload id to search for. This value will be 
 *              take in consideration only for multiple attributes like 
 *              "rtpmap", "fmtp", "AS", etc.
 * @return the attribute found or NULL.
 */
  sdp_attribute_t *sdp_message_att_find(sdp_message_t * sdp, int pos_media, 
										char *field, char *payload_id);
/**
 * Get a media position on SDP packet.
 * @param sdp The element to work on.
 * @param media_id The media id to be found.
 * @return position of searching media on SDP packet.
 */
int sdp_message_get_media_pos(sdp_message_t * sdp, 
							  char *media_id);
/* </UOLFONE> */
