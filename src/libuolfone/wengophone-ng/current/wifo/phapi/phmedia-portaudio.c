...
/* <UOLFONE> */
  // "framesize" was calculated considering packetization used on incoming 
  // data traffic that is given by "as->in_ptime" field.
  // This value can be used "as is" for the output device (considering that 
  // incoming network data goes to output device, like SPK).
  // Now we must calculate here the framesize relative to outgoing data that 
  // is given by "as->out_ptime" so we can use it to initialize the input 
  // device, like MIC.
  in_framesize = (framesize / as->in_ptime) * as->out_ptime;
/* </UOLFONE> */
...
/* <UOLFONE> */
  pd->istream  = pa_dev_open(as, PH_PA_INPUT, name, rate, in_framesize, ph_pa_latency);
/* </UOLFONE> */
...
/* <UOLFONE> */
  pd->ostream  = pa_dev_open(as, PH_PA_OUTPUT, name, rate, out_framesize, ph_pa_latency);
/* </UOLFONE> */
