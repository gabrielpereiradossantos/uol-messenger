...
int ph_astream_decoded_framesize_get(phastream_t *stream, int channel)
{
	int framesize = stream->ms.codec->decoded_framesize;

	if ( (channel == 0) && (stream->out_ptime) )
	{
		// Default ptime is 20ms, modify the framesize to manage the new ptime
		framesize = framesize * stream->out_ptime / 20;
	}
	else if ( (channel == 1) && (stream->in_ptime) )
	{
		// Default ptime is 20ms, modify the framesize to manage the new ptime
		framesize = framesize * stream->in_ptime / 20;
	}
...
/* <UOLFONE <pTime adjustment> > */
  ...
  int framesize = ph_astream_decoded_framesize_get(stream, 1);
/* </UOLFONE> */
...
/* <UOLFONE <pTime adjustment> > */
      ...
      //		lg = info.bytes - 2*codec->ph_astream_decoded_framesize_get(stream, 1);
/* </UOLFONE> */
      lg = 2*framesize;
...
/* <UOLFONE <pTime adjustment> > */
  ...
  int internal_framesize = ph_astream_decoded_framesize_get(stream, 1);
/* </UOLFONE> */
...
/* </UOLFONE> */
  ...
  {
/* <UOLFONE <pTime adjustment> > */
		stream->ms.rxts_inc += ph_astream_decoded_framesize_get(stream, 1)/2;
/* </UOLFONE> */
    return 0;
  }
...
/* <UOLFONE <pTime adjustment> > */
        ...
        if (morebytes > ph_astream_decoded_framesize_get(stream, 1))
        {
            //morebytes = codec->decoded_framesize;
            morebytes = ph_astream_decoded_framesize_get(stream, 1);
        }
/* </UOLFONE> */
...
#endif
	*/
	char dummy_input_buf[1000];
	void* codec_input_buf = recordbuf;

	if (phGetUOLFoneMicMute())
	{
		memset(dummy_input_buf, 0x01, 1000);
		codec_input_buf = dummy_input_buf;
	}

#ifdef PH_FORCE_16KHZ
    if (stream->clock_rate == 16000)
    {
      enclen = codec->encode(stream->ms.encoder_ctx, codec_input_buf, framesize, data_out_enc, sizeof(data_out_enc));
    }
    else
    {
      DBG_DYNA_AUDIO_TX("downsampling for FORCE_16KHZ mode (%d)->(%d)\n", framesize, framesize/2);
      ph_downsample(stream->resamplectx, codec_input_buf, framesize);
      enclen = codec->encode(stream->ms.encoder_ctx, codec_input_buf, framesize/2, data_out_enc, sizeof(data_out_enc));
      framesize /= 2;
    }
#else
    DBG_DYNA_AUDIO("DYNA_AUDIO:ph_encode_and_send_audio_frame: start encoding\n");
...
/* <UOLFONE <pTime adjustment> > */
  ...
  int codec_tx_framesize = ph_astream_decoded_framesize_get(stream, 0);
/* </UOLFONE> */
  ...
  if (stream->ms.running)
  {
    ...
    if ((playbuf != 0) && playbufsize && (*playbufsize > 0))
    {
        ...
        *playbufsize = i;
		...
		/* <UOLFONE> */
		if (phcfg.rtp_receive_timeout > 0)
		{
			if (0 == stream->firstCheck)
			{
				if (0 == i)
				{
					/* Perform RTP packet receiving checking */
					struct timeval now, diff;
					gettimeofday(&now, 0);

					ph_tvdiff(&diff, &now, &(stream->last_rtp_recv_time));
					
					if (diff.tv_sec > phcfg.rtp_receive_timeout)
					{
						phmstream_t* ms = &(stream->ms);

						DBG_MEDIA_ENGINE("ph_audio_callback: RTP receive timeout reached!\n");

						if ((stream->streamReceiveTimeoutCbk) &&
							(ms->mses) &&
							((ms->mses)->cbkInfo) &&
							stream->ms.running)
						{
							(stream->streamReceiveTimeoutCbk)((ms->mses)->cbkInfo, phSTREAMAUDIORECEIVETIMEOUT);
						}

						gettimeofday(&(stream->last_rtp_recv_time), 0);
					}
				}
			}
			else
...
/* <UOLFONE <pTime adjustment> > */
  //int internal_framesize = codec->decoded_framesize;
  int internal_framesize = ph_astream_decoded_framesize_get(stream, 0);
/* </UOLFONE> */
  int internal_clockrate = stream->clock_rate;
  int i;
...
/* <UOLFONE <pTime adjustment> > */
  //int framesize = stream->ms.codec->decoded_framesize;
  // Change on value "1" informed as the second parameter will demand changes on 
  // method "pa_stream_open" of "phmedia_portaudio.c" file.
  int framesize = ph_astream_decoded_framesize_get(stream, 1);
/* </UOLFONE> */
...
/* <UOLFONE <pTime adjustment> > */
      ...
      ph_astream_decoded_framesize_get(stream, 1),
/* </UOLFONE> */
      clockrate,
      framesize,
      stream->actual_rate);
...
/* <UOLFONE <pTime adjustment> > */
    ...
    stream->lastframe = calloc(ph_astream_decoded_framesize_get(stream, 1), 1);
/* </UOLFONE> */
  }
...
/* <UOLFONE <pTime adjustment> > */
    ...
    stream->ms.media_io_timer_impl->timer_set_delay(stream->ms.media_io_timer, stream->in_ptime);
/* </UOLFONE> */
#endif
...
/* <UOLFONE <pTime adjustment> > */
    //int framesize = stream->ms.codec->decoded_framesize;
    int framesize = ph_astream_decoded_framesize_get(stream, 0);
/* </UOLFONE> */
    int clockrate = stream->clock_rate;
...
/* <UOLFONE <pTime adjustment> > */
  ...
  {
    if ( (sp->ipayloads[iPayloadIndex].number == codecpt) && 
        (sp->ipayloads[iPayloadIndex].ptime != 0) )
    {
      stream->in_ptime = sp->ipayloads[iPayloadIndex].ptime;
      
      break;
    }
  }
  
  if (stream->in_ptime == 0)
  {
    stream->in_ptime = atoi(DEFAULT_PTIME_VALUE);
  }
  ...
  for (iPayloadIndex = 0; iPayloadIndex < PH_MSTREAM_PARAMS_MAX_PAYLOADS; iPayloadIndex++)
  {
    ...
    {
      stream->out_ptime = sp->opayloads[iPayloadIndex].ptime;
      
      break;
    }
  }
  
  if (stream->out_ptime == 0)
  {
    stream->out_ptime = sp->ptime;
  }
  
  if (stream->out_ptime == 0)
  {
    stream->out_ptime = atoi(DEFAULT_PTIME_VALUE);
  }
/* </UOLFONE> */
/* </UOLFONE> */
  ...
  if (sp->flags & PH_MSTREAM_FLAG_TUNNEL)
  {
	...
	*/
	...
	if (sp->tunRtp && sp->tunRtcp)
	{
		tun = (RtpTunnel*) sp->tunRtp;
		tun2 = (RtpTunnel*) sp->tunRtcp;
  ...
  /* <UOLFONE> */
  /* Set stream receiving timeout callback */
  stream->streamReceiveTimeoutCbk = s->streamReceiveTimeoutCbk;
  stream->firstCheck = 1;
  /* </UOLFONE> */
    ...
    if (stream->ms.payload ==  sp->ipayloads[0].number)
    {
      ...
      if (stream->ms.tunRtp)
      {
		...
		*/
		if (sp->tunRtp && sp->tunRtcp)
		{
			newTun = (RtpTunnel*) sp->tunRtp;
			newTun2 = (RtpTunnel*) sp->tunRtcp;
...
/* <UOLFONE> */
int ph_media_audio_cleanup()
{
  if (ph_audio_mux != NULL)
  {
	g_mutex_free(ph_audio_mux);
	ph_audio_mux = NULL;
  }
