...
/* <UOLFONE> */
#ifdef _MSC_VER
#define UOLFONECLIENT_UA	"UOLFoneClient"
#endif
/* <UOLFONE> */
...
/* <UOLFONE> */
/**
 * Request media stream address renegotiation
 *
 * @param cid          call id of call.
 * @return             0 in case of success
 */
MY_DLLEXPORT int phRenegotiateCallMedia(int cid);
/* </UOLFONE> */
...
/* <UOLFONE> */
typedef struct phCustomHeaderInfo phCustomHeaderInfo_t;

struct phCustomHeaderInfo {
  char *name;
  char *value;
  struct phCustomHeaderInfo *next;
  struct phCustomHeaderInfo *parent;
};

struct phRegStateInfo {
  int regid;
  int regStatus;
  struct phCustomHeaderInfo *custom_headers;
};
 ...
 */
enum phStreamError {
  phSTREAMAUDIORECEIVETIMEOUT,   /* audio receiving timeout */
  phSTREAMVIDEORECEIVETIMEOUT    /* video receiving timeout */
};

/**
 * @struct phConfEvent
 */
struct phCallStreamErrorInfo {
  enum phStreamError error;
  int   vlid;                  /*! virtual line id */
};
typedef struct phCallStreamErrorInfo phCallStreamErrorInfo_t;
...
struct phCallbacks {
  ...
  /* <UOLFONE> */
  /* -- added extra info for registration progress
  void  (*regProgress)(int regid, int regStatus);                       /*!< registration status (0 - OK, else SIP error code */
  void  (*regProgress)(int regid, const phRegStateInfo_t *info);
  /* </UOLFONE> */
...
/* <UOLFONE> */
/**
 * Configure tunnel proxy parameters: This funtion should be call just before phInit, when the global variable phcfg 
 *									is properly set up.
 *
 * @param http_proxy			    IP address of http proxy in local network, set to 0 if there is no local proxy
 * @param http_proxy_port				Port of local http proxy
 ...
 * @param proxy_passwd				Password for proxy
*/
MY_DLLEXPORT int phTunnelProxyConfig(const char* http_proxy, const int http_proxy_port,
                  const char *proxy_user, const char* proxy_passwd);

/**
 * Configure media tunnel parameters: This funtion should be call just before phInit, when the global variable phcfg 
 *									is properly set up.
 *
 * @param httpt_server				IP address of the HTTP TUNNEL SERVER that do the http to udp conversion
 * @param httpt_server_port			Port of the HTTP TUNNEL SERVER that do the http to udp conversion
 ...
 *									setup a tunnel with given parameters.
*/
MY_DLLEXPORT int phMediaTunnelConfig(const char* httpt_server, const int httpt_server_port,
...
typedef struct phAuthInfo phAuthInfo_t;
/* </UOLFONE> */
...
#define PH_TUNNEL_USE  1
 ...
 /* <UOLFONE> */
  phAuthInfo_t auth_info;

  char user_agent[128];
  int rtp_use_tunnel;
  char rtp_httpt_server[128];
  int  rtp_httpt_server_port;
  int rtp_receive_timeout;  /*!< timeout for no RTP packet receiving in seconds (when 0 no-check) */
  char local_media_ip[128];
  char uol_library_search_path[256];

  int tunnel_keep_alive_timeout;
  int rtp_tunnel_accept_keep_alive_timeout;
  int rtp_tunnel_connect_keep_alive_timeout;
  int rtp_tunnel_connection_accept_timeout;
 /* <UOLFONE> */
};
#else
struct phConfig {
...
#define PH_TUNNEL_USE		1
 ...
 /* <UOLFONE> */
  ...
  char rtp_httpt_server[128];
  int  rtp_httpt_server_port;
  int rtp_receive_timeout;  /*!< timeout for no RTP packet receiving (when 0 no-check) */
  char local_media_ip[128];
  char uol_library_search_path[256];
