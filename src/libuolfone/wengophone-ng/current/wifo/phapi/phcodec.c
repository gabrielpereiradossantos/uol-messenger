...
ph_ilbc_negotiate_attrib(char *attr_name, char *local_attr_value, char *remote_attr_value)
{
  ...
  {
    int local_mode = 20;	// Default local packetization.
    int remote_mode = 0;
    ...
    else if ( (local_attr_value == NULL) && (remote_mode_str != NULL) )
    {
      ...
      strncat(final_mode_str, DEFAULT_PTIME_VALUE, 2);
...
/* <UOLFONE> */
  ph_ilbc_encode, ph_ilbc_decode, NULL, ph_ilbc_negotiate_attrib
/* </UOLFONE> */
};
...
/* <UOLFONE> */
struct ph_codec_plugin
{
	HLIB lib;
	struct ph_codec_plugin * next;
};

static struct ph_codec_plugin * list_codec_plugins = NULL;

void init_codec_plugin_list()
{
	list_codec_plugins = NULL;
}

void add_codec_plugin(HLIB lib)
{
	if (lib != NULL)
	{
		struct ph_codec_plugin * new_codec_plugin = (struct ph_codec_plugin *) malloc(sizeof(struct ph_codec_plugin));

		if (new_codec_plugin != NULL)
		{
			memset(new_codec_plugin, 0, sizeof(*new_codec_plugin));
			
			new_codec_plugin->lib = lib;

			if (NULL == list_codec_plugins)
			{
				list_codec_plugins = new_codec_plugin;
				new_codec_plugin->next = NULL;
			}
			else
			{
				struct ph_codec_plugin * codec_plugin = list_codec_plugins;

				while ((codec_plugin != NULL) && 
					   (codec_plugin->next != NULL))
				{
					codec_plugin = codec_plugin->next;
				}

				new_codec_plugin->next = codec_plugin->next;
				codec_plugin->next = new_codec_plugin;
			}
		}
	}
}

void release_codec_plugin_list()
{
	struct ph_codec_plugin * codec_plugin = NULL;

	while (list_codec_plugins != NULL)
	{
		codec_plugin = list_codec_plugins;
		
		list_codec_plugins = list_codec_plugins->next;

		dlclose(codec_plugin->lib);
		free(codec_plugin);
	}
}
