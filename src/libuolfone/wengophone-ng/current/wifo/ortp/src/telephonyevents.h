...
#ifdef MULTI_SSRC
/* low level functions */	
gint rtp_source_telephone_events_supported(RtpSource *session);

mblk_t	*rtp_source_create_telephone_event_packet(RtpSource *session, int start);

gint rtp_source_read_telephone_event(RtpSource *src,
									  mblk_t *packet,telephone_event_t **tab);

/* high level functions*/
gint rtp_source_send_dtmf(RtpSource *src, gchar dtmf, guint32 userts);
gint rtp_source_send_dtmf2(RtpSource *src, gchar dtmf, guint32 userts, int duration);

/* for high level telephony event callback */
void rtp_source_check_telephone_events(RtpSource *src, mblk_t *m0);
