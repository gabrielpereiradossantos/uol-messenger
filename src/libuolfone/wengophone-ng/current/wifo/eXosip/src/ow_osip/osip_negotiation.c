										   ...
										   int bIsIncomingCall)
{
  ...
  if (remote_media_pos != -1)
  {
        ...
        remote, remote_media_pos, attrib_index)) != NULL)
    {
      ...
      if (strcmp(remote_attrib->a_att_field, "ptime") == 0)
      {
            ...
            remote_attrib->a_att_field, NULL)) == NULL)
	    {
          ...
          else
          {
            sdp_message_a_attribute_add(local, local_media_index, 
                osip_strdup(remote_attrib->a_att_field), 
                osip_strdup(DEFAULT_PTIME_VALUE));
          }
	    }
	    else
	    {
      ...
      {
        // First we get the payload to compare with local existent payloads.
        ...
        {
          int bAddAttrib = 0;
          char *local_attr_value = NULL;
          char *final_attr_value = NULL;
          phcodec_t *pCodec = NULL;
          
          pCodec = ph_media_lookup_codec(atoi(payload_id));
          
          if ( (pCodec) && (pCodec->negotiate_attrib) )
          {
            if ((local_attrib = sdp_message_att_find(local, local_media_index, 
                remote_attrib->a_att_field, payload_id)) != NULL)
            {
              local_attr_value = local_attrib->a_att_value;
            }
            
            // If some value is returned on "final_attr_value", it indicates 
			// the attribute value negotiated and that must be placed on 
			// local SDP. This can be the remote value itself or the result 
			// of some sort of calculation considering local and remote 
			// values.
			final_attr_value = pCodec->negotiate_attrib(
                remote_attrib->a_att_field, local_attr_value, 
                remote_attrib->a_att_value);
            
            if ( (final_attr_value) && 
                ( (!local_attr_value) || 
                    (strcmp(final_attr_value, local_attr_value) != 0) ) )
            {
              if (local_attr_value)
              {
              ...
              sdp_message_a_attribute_add(local, local_media_index, 
                  osip_strdup(remote_attrib->a_att_field), 
                  osip_strdup(final_attr_value));
              
              osip_free(final_attr_value);
            }
          }
        }
...
/* <UOLFONE> */
					  osip_negotiation_ctx_t * context, int bIsIncomingCall)
/* </UOLFONE> */
{
...
/* <UOLFONE> */
		  osip_negotiation_execute_media_negotiation(
			  local, med, i, remote, bIsIncomingCall);
/* </UOLFONE> */
	    }
