...
/* <UOLFONE> */
int eXosip_sdp_set_attributes(osip_negotiation_ctx_t *context, sdp_message_t *sdp, int pos)
{
	if (eXosip.media_use_tunnel)
	{
		sdp_message_a_attribute_add(sdp, -1, osip_strdup("tunneling"), osip_strdup("1"));
	}
...
int eXosip_sdp_negotiation_init(osip_negotiation_t **sn)
{
  ...
  /* <UOLFONE> */
  osip_negotiation_set_fcn_set_attributes(*sn, &eXosip_sdp_set_attributes);
  /* </UOLFONE> */
...
int eXosip_get_sdp_audio_ptime(sdp_message_t *sdp)
{
	int pos;
...
/* <UOLFONE> */
int eXosip_get_sdp_media_format_list(sdp_message_t *sdp, osip_list_t *fmtp_attribs_list)
{
	int pos;
	
	if (fmtp_attribs_list == NULL)
	{
		return -1;
	}
	...
	for (pos = 0; !sdp_message_endof_media(sdp, pos); pos++)
	{
		char *media_type = sdp_message_m_media_get(sdp, pos);
		
		// Looking for the audio media field
		if (osip_strncasecmp(media_type, "audio", 5) == 0)
		{
			int i;
			sdp_attribute_t *att;
			...
			for (i = 0; (att = sdp_message_attribute_get(sdp, pos, i)); i++)
			{
				...
				{
					osip_list_add(fmtp_attribs_list, att->a_att_value, -1);
				}
			}
		}
	}
...
/*
 * @param source_id  From which SDP info must be retrieved. "0" indicates local SDP 
 *                   and "1" remote SDP.
 * @param is_incomming_call  Indicates whether we should get "ptime" considering 
 *                           a receiveing call or a call initiated here.
 */
int eXosip_get_media_formats_from_call(eXosip_event_t* je, 
		int jid, int source_id, int is_incomming_call, osip_list_t *fmtp_attrib_list)
{
  eXosip_call_t *jc = eXosip_get_call(je, jid);
  ...
  if (source_id == 0)
  {
    return eXosip_get_sdp_media_format_list(
        osip_negotiation_ctx_get_local_sdp(jc->c_ctx), fmtp_attrib_list);
  }
  else if (is_incomming_call == 0)
  {
    return eXosip_get_sdp_media_format_list(
        osip_negotiation_ctx_get_remote_sdp(jc->c_ctx), fmtp_attrib_list);
  }
  else
  {
    return eXosip_get_sdp_media_format_list(
        eXosip_get_remote_sdp(jc->c_inc_tr), fmtp_attrib_list);
  }
}
/* </UOLFONE> */
