...
eXosip_event_free(eXosip_event_t *je)
{
  ...
  /* <UOLFONE> */
  eXosip_event_clear_custom_reply_headers(je);
  /* </UOLFONE> */
...
/* <UOLFONE> */
int eXosip_event_add_custom_reply_headers(eXosip_event_t *je, osip_message_t *sip)
{
	if (!je || !sip)
	{
		return -1;
	...
	if (&sip->headers)
	{
		...
		if (!je->custom_reply_headers)
		{
			je->custom_reply_headers = (osip_list_t *) osip_malloc (sizeof (osip_list_t));
			osip_list_init(je->custom_reply_headers);
		}
		...
		while (!osip_list_eol(&sip->headers, pos))
		{
			...
			if (0 == i)
			{
				osip_list_add(je->custom_reply_headers, header2, -1);
			}
...
int eXosip_event_clear_custom_reply_headers(eXosip_event_t *je)
{
	if (je)
	{
		if (je->custom_reply_headers)
		{
			...
			while (!osip_list_eol (je->custom_reply_headers, pos))
			{
				header = (osip_header_t *) osip_list_get(je->custom_reply_headers, pos);
				osip_list_remove(je->custom_reply_headers, pos);
				osip_header_free(header);
			}

			osip_free (je->custom_reply_headers);
			je->custom_reply_headers = NULL;
		}
