...
/* <UOLFONE> */
char * eXosip_get_mediaip()
{
	return eXosip.mediaip;
}
/* </UOLFONE> */
	      ...
	      if (0 == osip_strncasecmp (tmp, "audio", 5))
		{
		  ...
		  do {
			...
			sdp_message_a_attribute_add (sdp,
			  ...
			  /* <UOLFONE> */
			  /* declare G729 without annex B */
			else if (0==strcmp("18",payload))
			{
			sdp_message_a_attribute_add (sdp,
						     pos,
							 osip_strdup ("fmtp"),
						     osip_strdup ("18 annexb=no"));
			}
			  /* </UOLFONE> */
		    k++;
		  } while (payload != NULL);
...
/* <UOLFONE> */
int eXosip_set_media_use_tunnel(int use)
{
	eXosip.media_use_tunnel = use;
...
int eXosip_set_uol_library_search_path(const char* search_path)
{
	return uolfone_auth_set_library_path(search_path);
}
...
/* <UOLFONE> */
eXosip_call_t* eXosip_get_call(eXosip_event_t* je, int jid)
{
  eXosip_dialog_t *jd = NULL;
  eXosip_call_t *jc = NULL;
  
  if ( (je != NULL) && (je->jc != NULL) )
  {
    return je->jc;
  }
 ...
 */
int eXosip_get_audio_ptime_from_call(eXosip_event_t* je, 
		int jid, int source_id, int is_incomming_call)
{
  eXosip_call_t *jc = eXosip_get_call(je, jid);
  
  if (jc == NULL)
  {
	 return atoi(DEFAULT_PTIME_VALUE);
  }
