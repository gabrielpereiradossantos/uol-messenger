...
class CObserverRegister
{
public:
	virtual void RegisterObserver(TObserver* pObserver)
	{
		if (pObserver != NULL)
		{
	...
	virtual void UnregisterObserver(TObserver* pObserver)
	{
		if (pObserver != NULL)
		{
			CComCritSecLock<CComAutoCriticalSection> lock(m_csObservers);
...
protected:
	CAtlList<TObserver*>		m_listObservers;
