...
BOOL CPictureExWnd::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	...
	HRSRC hPicture = FindResource(ATL::_AtlBaseModule.GetResourceInstance(),szResourceName,szResourceType);
	HGLOBAL hResData;
	if (!hPicture || !(hResData = LoadResource(ATL::_AtlBaseModule.GetResourceInstance(),hPicture)))
	{
		ATLTRACE(_T("Load (resource): Error loading resource %s\n"),szResourceName);
		return FALSE;
	};
	DWORD dwSize = SizeofResource(ATL::_AtlBaseModule.GetResourceInstance(),hPicture);
...
LRESULT CPictureExWnd::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) 
{
	...
	if (nPaintWidth > 0)
	{
		...
		if (m_bStretchImage)
		{
			::StretchBlt(hDC, 0, 0, nPaintWidth, nPaintHeight,	
				m_hMemDC, m_PaintRect.left, m_PaintRect.top, 
				m_PictureSize.cx, m_PictureSize.cy, SRCCOPY);
		}
		else
		{
...
void CPictureExWnd::SetStretchImage(BOOL bStretchImage)
{
	m_bStretchImage = bStretchImage;
}
