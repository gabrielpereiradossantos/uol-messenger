...
LRESULT CConfigDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	m_LogMessageEnabledButton = GetDlgItem(IDC_CHECK_DONT_LOG_MESSAGE);
	m_LogMessageEnabledButton.SetCheck(!m_pCryptoPluginConfig->IsLogMessageEnabled());
...
LRESULT CConfigDlg::OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	m_pCryptoPluginConfig->SetLogMessageEnabled(!m_bLogMessageEnabled);
	m_pCryptoPluginConfig->SaveCryptoConfig();
	CMessageWindowMonitor::GetInstance()->UpdateCryptoLogMessageConfig();
