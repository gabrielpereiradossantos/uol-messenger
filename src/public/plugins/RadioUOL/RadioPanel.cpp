...
int CRadioPanel::TIMER_SHOW_NOTIFY_POPUP = 0x387;


CRadioPanel::CRadioPanel(const CString& strPanelName, const CString& strPluginInternalName) : 
...
CString CRadioPanel::GetPanelName() const
{
	return m_strPanelName;
}


CString CRadioPanel::GetPluginInternalName() const
{
	return m_strPluginInternalName;
}


void CRadioPanel::GetMinSize(CSize& size)
{
	size.cx = 0;
	size.cy = 80;
}
...
HWND CRadioPanel::CreatePanel(HWND hWndParent, const CRect& rectPanel, DWORD dwWindowStyle)
{
...
void CRadioPanel::DestroyPanel(HWND hWndPanel)
{
	if (hWndPanel == m_hWnd)
	{
		DestroyWindow();
...
IUOLMessengerTooltipProviderPtr CRadioPanel::GetTooltipProvider() const
{
	return m_pTooltipProvider;
}


LRESULT CRadioPanel::OnCreate(LPCREATESTRUCT lpCs)
{
	CRect rectClient;
	...
	if (pUIManager)
	{
		m_pIcon = pUIManager->LoadImage(ATL::_AtlBaseModule.GetModuleInstance(), IDB_RADIO_LIST_ENTRY);
...
void CRadioPanel::OnDestroy()
{
	if (m_nTimerNotifyPopUp)
	{
...
void CRadioPanel::OnSize(UINT nType, const CSize& size)
{
...
void CRadioPanel::OnTimer(UINT nTimerId, TIMERPROC /*timerProc*/)
{
	if (TIMER_SHOW_NOTIFY_POPUP == nTimerId)
	{
		...
		if (pFacade)
		{
			...
			if (pNotifyPopUpManager)
			{
				...
				pNotifyPopUpManager->ShowAlertWindow(_T("UOL Messenger - Rádio UOL"), 
						listLines, m_pIcon, new CRadioNotifyPopUpHyperLinkHandler());
			}
		}
	}
}
...
void CRadioPanel::OnClickedButtonTest(UINT /*uCode*/, int nID, HWND /*hwndCtrl*/)
{
	CString strMessage;
