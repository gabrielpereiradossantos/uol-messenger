...
#include "IUOLMessengerNotifyPopUpHandler.h"


class __declspec(novtable) IUOLMessengerNotifyPopUpManager
{
public:
	enum ShowTimeout
	{
		SHOW_TIMEOUT_INFINITE = -2,
		SHOW_TIMEOUT_DEFAULT = -1
	};

public:
	virtual ~IUOLMessengerNotifyPopUpManager() { }
	...
	virtual void ShowAlertWindow(const CString& strTitle, 
								 ...
								 DWORD dwShowTimeout = SHOW_TIMEOUT_DEFAULT) = 0;

	virtual void ShowAlertWindow(IUOLMessengerNotifyPopUpHandlerPtr pNotifyPopUpHandler,
								 DWORD dwShowTimeout = SHOW_TIMEOUT_DEFAULT) = 0;
};

MAKEAUTOPTR(IUOLMessengerNotifyPopUpManager);