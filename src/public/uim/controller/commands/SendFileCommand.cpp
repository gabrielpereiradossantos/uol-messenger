...
CSendFileCommand::CSendFileCommand(IUOLMessengerContactPtr pContact, CWindow wnd) : 
	m_pContact(pContact),
	m_wnd(wnd)
{
}
...
CSendFileCommand::~CSendFileCommand()
{
}


void CSendFileCommand::Execute()
{
	CString strMessage;
	...
	if (m_pContact)
	{
		...
		CString strUserDir = _T("\\");

		CString strLastFolder = CUIMApplication::GetApplication()->GetSettingsManager()->GetGeneralSettings()->GetLastSendFileFolder();

		if (FALSE == strLastFolder.IsEmpty())
		{
			strUserDir = strLastFolder;
		}

		CString strCaption;
		strCaption.LoadString(IDS_SEND_FILE_DIALOG_CAPTION);

		fileDialog.m_ofn.lpstrInitialDir = strUserDir;
		fileDialog.m_ofn.lpstrTitle = strCaption;
		...
		do {
			...
			{ 
				if (fileDialog.m_szFileName != NULL)
				{
					struct __stat64 stFileInfo = {0};

					int nStat = ::_tstat64(fileDialog.m_szFileName, &stFileInfo);
					...
					if (0 == nStat)
					{
						if (stFileInfo.st_size > 0)
						{
							...
							strLastFolder = fileDialog.m_szFileName;
							...
							CUIMApplication::GetApplication()->GetFileTransferManager()->SendFile(m_pContact->GetAccount(), m_pContact->GetName(), fileDialog.m_szFileName);
						}
						else
						{
