...
CLeaveChatCommand::CLeaveChatCommand(IUOLMessengerConversationPtr pConversation) :
    m_pConversation(pConversation)
{
}


CLeaveChatCommand::~CLeaveChatCommand(void)
{
}


void CLeaveChatCommand::Execute()
{
	...
	if (pProtocol5 && pProtocol5->HasChatSupport())
	{
        ...
        IMProtocolError imError = pProtocol5->ChatLeave((LPCTSTR) m_pConversation->GetAccount()->GetUserId(),  nChatId);
