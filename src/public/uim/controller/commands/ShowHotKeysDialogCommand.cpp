...
#include "../UIManager.h"
#include "../../view/HotKeysDlg.h"
#include "../../view/Skin/ElementBuilder.h"
#include "../../view/Skin/MainDialogElement.h"

CShowHotKeysDialogCommand::CShowHotKeysDialogCommand(void)
{
}
...
void CShowHotKeysDialogCommand::Execute()
{
	CHotKeysDlgPtr pDialog;

	CUIManagerPtr pUIManagerPtr = ap_dynamic_cast<CUIManagerPtr>(CUIMApplication::GetApplication()->GetUIManager());
	IElementPtr pElement = pUIManagerPtr->GetWindowManager()->GetElement("__HotKeysDlgBuilderInstance");			
	if (pElement)
	{
		...
		if (pBuilder)
		{
			pDialog = ap_dynamic_cast<CHotKeysDlgPtr>(pBuilder->CreateElement());			
