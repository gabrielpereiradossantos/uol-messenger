...
CUnblockContactCommand::CUnblockContactCommand(IUOLMessengerContactPtr pContact, CWindow wnd) :
...
CUnblockContactCommand::CUnblockContactCommand(IUOLMessengerAccountPtr pAccount, const CString& strContact, CWindow wnd) : 
...
CUnblockContactCommand::~CUnblockContactCommand()
{
}


void CUnblockContactCommand::Execute()
{
	...
	if (pProtocol && pProtocol->HasContactBlockingSupport())
	{
		...
		if (!(pProtocol->GetPrivacyFlags() & PRIVACY_FLAGS_HAS_PERMIT_LIST))
		{
			strMessage.Format(IDS_ADD_TO_LIST, m_strContactName);

			if (CUIMApplication::GetApplication()->GetUIManager()->ShowMessageBox(m_wnd, strMessage, MB_YESNO | MB_ICONQUESTION) == IDYES)
			{
				pProtocol->RemoveDenyContact((LPCTSTR) strUser, (LPCTSTR) m_strContactName);
				
				CUIMApplication::GetApplication()->GetUIManager()->ShowContactDlg(m_pAccount, m_strContactName);
