...
CShowAboutDialogCommand::CShowAboutDialogCommand(void)
{
}
...
void CShowAboutDialogCommand::Execute()
{
	CAboutDlgPtr pDialog;

	CUIManagerPtr pUIManagerPtr = ap_dynamic_cast<CUIManagerPtr>(CUIMApplication::GetApplication()->GetUIManager());
	IElementPtr pElement = pUIManagerPtr->GetWindowManager()->GetElement("__AboutDlgBuilderInstance");			
	if (pElement)
	{
		...
		if (pBuilder)
		{
			pDialog = ap_dynamic_cast<CAboutDlgPtr>(pBuilder->CreateElement());			
