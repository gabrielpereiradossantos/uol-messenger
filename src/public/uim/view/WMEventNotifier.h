...
enum
{
	...
	WM_ON_KEYDOWN_EVENT_NOTIFIER,
	WM_ON_KEYUP_EVENT_NOTIFIER
};
...
protected:
	...
	virtual void OnKeyDownEventNotifier(UINT nKey) {};
	virtual void OnKeyUpEventNotifier(UINT nKey) {};
	...
	inline LRESULT OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if (m_bSendNewMessage)
		{
			if (m_hNotifyingWnd)
			{
				PostMessage(m_hNotifyingWnd, WM_ON_CHAR_EVENT_NOTIFIER, wParam, lParam);
			}
			else if (m_pT != NULL)
			{
				PostMessage(m_pT->m_hWnd, WM_ON_CHAR_EVENT_NOTIFIER, wParam, lParam);
			}
		}
		else
		{
	...
	inline LRESULT OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		ATLTRACE(_T("CWMEventNotifier::OnKeyDown()\n"));
		
		if (m_bSendNewMessage)
		{
			if (m_hNotifyingWnd)
			{
				PostMessage(m_hNotifyingWnd, WM_ON_KEYDOWN_EVENT_NOTIFIER, wParam, lParam);
			}
			else if (m_pT != NULL)
			{
				PostMessage(m_pT->m_hWnd, WM_ON_KEYDOWN_EVENT_NOTIFIER, wParam, lParam);
			}
		}
		else
		{
			OnKeyDownEventNotifier((UINT)wParam);
		}
	...
	inline LRESULT OnKeyUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		ATLTRACE(_T("CWMEventNotifier::OnKeyUp()\n"));
		
		if (m_bSendNewMessage)
		{
			if (m_hNotifyingWnd)
			{
				PostMessage(m_hNotifyingWnd, WM_ON_KEYUP_EVENT_NOTIFIER, wParam, lParam);
			}
			else if (m_pT != NULL)
			{
				PostMessage(m_pT->m_hWnd, WM_ON_KEYUP_EVENT_NOTIFIER, wParam, lParam);
			}
		}
		else
		{
			OnKeyUpEventNotifier((UINT)wParam);
		}
