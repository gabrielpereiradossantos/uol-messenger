...
class CWizardICQAccountDialog : public CUOLPropPageImpl,
    public CWinDataExchange<CWizardICQAccountDialog>
{
public:
    enum { IDD = IDD_DIALOG_WIZARD_ICQ_ACCOUNT };
	DECLARE_ELEMENT_NAME("WizardICQAccountDialog");

	CWizardICQAccountDialog();

    BEGIN_MSG_MAP(CWizardICQAccountDialog)
		MSG_WM_CTLCOLORSTATIC(OnCtlColor) 
		COMMAND_HANDLER_EX(IDC_BUTTON_WIZARD_ICQ_ADD_ACCOUNT, BN_CLICKED, OnClickButton)
		COMMAND_HANDLER_EX(IDC_LIST_WIZARD_ICQ_ACCOUNTS, LBN_SELCHANGE, OnSelectChange)
		MSG_WM_INITDIALOG(OnInitDialog)
        CHAIN_MSG_MAP(CUOLPropPageImpl)
    END_MSG_MAP()

    BEGIN_DDX_MAP(CWizardICQAccountDialog)
		DDX_TEXT(IDC_EDIT_WIZARD_ICQ_ID, m_strUIN)
		DDX_TEXT(IDC_EDIT_WIZARD_ICQ_PASSWORD, m_strPassword)
    END_DDX_MAP()
	...
	BOOL CaptureICQAccounts();

protected:
	int GetDlgID(){	return IDD_DIALOG_WIZARD_ICQ_ACCOUNT; }
...
MAKEAUTOPTR(CWizardICQAccountDialog);