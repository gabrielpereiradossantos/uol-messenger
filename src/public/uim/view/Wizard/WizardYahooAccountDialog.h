...
class CWizardYahooAccountDialog : public CUOLPropPageImpl,
    public CWinDataExchange<CWizardYahooAccountDialog>
{
public:
    enum { IDD = IDD_DIALOG_WIZARD_YAHOO_ACCOUNT };
	DECLARE_ELEMENT_NAME("WizardYahooAccountDialog");

	CWizardYahooAccountDialog();
	~CWizardYahooAccountDialog();

    BEGIN_MSG_MAP(CWizardYahooAccountDialog)
        MSG_WM_INITDIALOG(OnInitDialog)
		MSG_WM_CTLCOLORSTATIC(OnCtlColor) 
		COMMAND_HANDLER_EX(IDC_BUTTON_WIZARD_YAHOO_ADD_ACCOUNT, BN_CLICKED, OnClickButton)
		COMMAND_HANDLER_EX(IDC_LIST_WIZARD_YAHOO_ACCOUNTS, LBN_SELCHANGE, OnSelectChange)
        CHAIN_MSG_MAP(CUOLPropPageImpl)
    END_MSG_MAP()

    BEGIN_DDX_MAP(CWizardYahooAccountDialog)
		DDX_TEXT(IDC_EDIT_WIZARD_YAHOO_ID, m_strID)
		DDX_TEXT(IDC_EDIT_WIZARD_YAHOO_PASSWORD, m_strPassword)
    END_DDX_MAP()
	...
	BOOL ValidateData(BOOL bIsButtonNext);

protected:
	int GetDlgID(){	return IDD_DIALOG_WIZARD_YAHOO_ACCOUNT; }
