...
class CWizardUOLAccountDialog : public CUOLPropPageImpl,
	public CWinDataExchange<CWizardUOLAccountDialog>
{
public:
	enum { IDD = IDD_DIALOG_WIZARD_UOL_ACCOUNT };
	DECLARE_ELEMENT_NAME("WizardUOLAccountDialog");

	CWizardUOLAccountDialog();
	~CWizardUOLAccountDialog();

	BEGIN_MSG_MAP(CWizardUOLAccountDialog)
		MSG_WM_INITDIALOG(OnInitDialog)
		MSG_WM_CTLCOLORSTATIC(OnCtlColor) 
		COMMAND_HANDLER_EX(IDC_BUTTON_WIZARD_UOLK_ADD_ACCOUNT, BN_CLICKED, OnClickButton)
		COMMAND_ID_HANDLER_EX(IDC_BUTTON_WIZARD_UOLK_SUBSCRIBE, OnSubscribeButtonClicked)
		COMMAND_HANDLER_EX(IDC_EDIT_WIZARD_UOLK_ID, EN_KILLFOCUS, OnKillFocus)
		CHAIN_MSG_MAP(CUOLPropPageImpl)
	END_MSG_MAP()

    BEGIN_DDX_MAP(CWizardUOLAccountDialog)
		DDX_TEXT(IDC_EDIT_WIZARD_UOLK_ID, m_strEmail)
		DDX_TEXT(IDC_EDIT_WIZARD_UOLK_PASSWORD, m_strPassword)
    END_DDX_MAP()
	...
	void OnSubscribeButtonClicked(UINT /*uCode*/, int nID, HWND hwndCtrl);
...
protected:
	int GetDlgID(){	return IDD_DIALOG_WIZARD_UOL_ACCOUNT; }
...
private:
	...
	CTransparentStatic m_staticSubscribe;
};

MAKEAUTOPTR(CWizardUOLAccountDialog);