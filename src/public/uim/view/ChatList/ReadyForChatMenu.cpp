...
CReadyForChatMenu::CReadyForChatMenu() :
	m_bBusy(FALSE)
{
}
...
IUOLMessengerContactPtr CReadyForChatMenu::GetContact()
{
	return m_pContact;
}


BOOL CReadyForChatMenu::IsBusy() const
{
	return m_bBusy;
}


void CReadyForChatMenu::CreateMenu(IUOLMessengerContactPtr pContact, IUOLMessengerConversation2Ptr pConversation)
{
	if ((pContact != NULL) && (!m_bBusy) && (pConversation != NULL))
	{
		m_pContact = pContact;
...
void CReadyForChatMenu::ShowMenu(const CPoint& ptPoint)
{
...
void CReadyForChatMenu::DestroyMenu()
{
	if (m_pMainMenu != NULL)
	{
...
CContextMenuPtr CReadyForChatMenu::GetContextMenu()
{
	return m_pMainMenu;
}


void CReadyForChatMenu::OnCommand(IUOLMessengerContextMenuItem* pItem)
{
	if (!m_pContact)
	{
		return;
	...
	if (pAccount)
	{
		...
		case CC_INVITE_FOR_CHAT:
			// Invite for chat

            IUOLMessengerCommandPtr pCommand = new CInviteForChatCommand(m_pConversation, m_pContact, FALSE);
...
void CReadyForChatMenu::CreateContextMenu()
{
	IUOLMessengerAccountPtr pAccount = m_pContact->GetAccount();
	...
	if (m_pMainMenu->CreateMenu())
	{
		...
		strMenuText = pStringLoader->GetValue("uol.resource.IDS_CONTACT_NODE_INVITE_FOR_CHAT_CAPTION");
        pMenuItem = new CContextMenuItem(CC_INVITE_FOR_CHAT, strMenuText, this);
...
void CReadyForChatMenu::AddItens(CAtlList<IUOLMessengerContextMenuItemPtr>& listItens)
{
...
void CReadyForChatMenu::AddSeparator()
{
	m_pMainMenu->AddSeparator();
}