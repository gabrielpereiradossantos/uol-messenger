				   ...
				   void *user_data)
{
	...
	if (ops != NULL && ops->request_input != NULL) {
		/* UOL << */
		/* Do not allocate PurpleRequestInfo struct only if (handle != NULL) */
		PurpleRequestInfo *info;
		void* ui_handle;

		ui_handle = ops->request_input(title, primary, secondary,
										default_value,
			 ...
			 void *user_data, va_list choices)
{
	...
	if (ops != NULL && ops->request_choice != NULL) {
		...
		ui_handle = ops->request_choice(title, primary, secondary,
						      default_value,
						      ok_text, ok_cb,
						      cancel_text, cancel_cb,
							  account, who, conv,
						  ...
						  void *user_data, size_t action_count, va_list actions)
{
	...
	if (ops != NULL && ops->request_action != NULL) {
		...
		ui_handle = ops->request_action(handle, title, primary, secondary,
										default_action, account, who, conv, 
					...
					void *user_data)
{
	...
	if (ops != NULL && ops->request_fields != NULL) {
		...
		ui_handle = ops->request_fields(title, primary, secondary,
				  ...
				  void *user_data)
{
	...
	if (ops != NULL && ops->request_file != NULL) {
		...
		ui_handle = ops->request_file(handle, title, filename, savedialog,
									  ok_cb, cancel_cb, 
									  account, who, conv, 
									  user_data);
				  ...
				  void *user_data)
{
	...
	if (ops != NULL && ops->request_file != NULL) {
		...
		ui_handle = ops->request_folder(title, dirname,
										ok_cb, cancel_cb, 
										account, who, conv, 
										user_data);
