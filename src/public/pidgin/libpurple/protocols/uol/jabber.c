...
GList *jabber_status_types(PurpleAccount *account)
{
	...
	type = purple_status_type_new_with_attrs(PURPLE_STATUS_AVAILABLE,
			...
			/* UOL << <custom status attribute> */
			"UOLFoneCapabilities", _("UOL Fone Capabilities"), purple_value_new(PURPLE_TYPE_STRING),
			/* UOL >> */
			NULL);
...
#define SET_REASON(x) \
	...
	if((error = xmlnode_get_child(packet, "error"))) {
		...
		/* UOL - custom tags */
		} else if(xmlnode_get_child(packet, "x-uol-auth-error-code")) {
			xmlnode *uolErrorCode = xmlnode_get_child(packet, "x-uol-auth-error-code");
			xmlnode *uolErrorText = xmlnode_get_child(packet, "x-uol-auth-error-msg");
			code = xmlnode_get_data(uolErrorCode);
			text = xmlnode_get_data(uolErrorText);
			/* <x-uol-auth-error-code>200</x-uol-auth-error-code><x-uol-auth-error-msg>invalid password</x-uol-auth-error-msg> */
		/* UOL */
...
{
	/* UOL <UOL Protocol Especific Change> << */
	//return TRUE;
	return FALSE;
	/* UOL >> */
}
...
void jabber_register_commands(void)
{
	purple_cmd_register("config", "", PURPLE_CMD_P_PRPL,
	                  ...
	                  /* UOL <UOL Protocol Especific Change> << */
	                  "prpl-uol", jabber_cmd_chat_config,
	...
	purple_cmd_register("part", "s", PURPLE_CMD_P_PRPL,
	                  PURPLE_CMD_FLAG_CHAT | PURPLE_CMD_FLAG_PRPL_ONLY |
	                  /* UOL <UOL Protocol Especific Change> << */
	                  PURPLE_CMD_FLAG_ALLOW_WRONG_ARGS, "prpl-uol",
	...
	purple_cmd_register("register", "", PURPLE_CMD_P_PRPL,
	                  PURPLE_CMD_FLAG_CHAT | PURPLE_CMD_FLAG_PRPL_ONLY,
	                  /* UOL <UOL Protocol Especific Change> << */
	                  "prpl-uol", jabber_cmd_chat_register,
