...
void jabber_presence_send(PurpleAccount *account, PurpleStatus *status)
{
	...
	PurpleStatus *tune;
	/* UOL << */
	JabberPresencePreSendCallback presence_pre_send_callback = NULL;
	/* UOL >> */
...
#define CHANGED(a,b) ((!a && b) || (a && a[0] == '\0' && b && b[0] != '\0') || \
    ...
    /* UOL >> */
		...
		if (presence)
		{
			presence_pre_send_callback = jabber_extension_get_presence_pre_send_callback();
			
			if (presence_pre_send_callback)
			{
				(presence_pre_send_callback)(presence, gc);
			}
		}
		/* UOL >> */
...
void jabber_presence_parse(JabberStream *js, xmlnode *packet)
{
	...
	xmlnode *caps = NULL;
	/* UOL */
	JabberPresenceReceivedCallback presence_received_callback = NULL;
	/* UOL */
	...
	if(jid->node && (chat = jabber_chat_find(js, jid->node, jid->domain))) {
		...
		/* UOL << */
		presence_received_callback = jabber_extension_get_presence_received_callback();
		
		if (presence_received_callback != NULL)
		{
			(presence_received_callback)(packet, b);
		}
		/* UOL >> */
		g_free(buddy_name);
	}
