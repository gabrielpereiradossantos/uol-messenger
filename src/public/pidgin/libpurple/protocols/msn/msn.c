...
/* UOL <chat specific changes> << */
    chat_name = g_strdup_printf(_("MSN Chat:%d"), ++chat_count);
...
/* UOL << */
		smileyShortcut = g_strdup(purple_smiley_get_shortcut(smiley));
		escapedSmileyShortcut = g_markup_escape_text(smileyShortcut, -1);
		
		// Find unescaped sortcut, then escaped
		ptr = g_strstr_len(msg, length, smileyShortcut);
		if (!ptr)
		{
			ptr = g_strstr_len(msg, length, escapedSmileyShortcut);
		}
		
		g_free(smileyShortcut);
		g_free(escapedSmileyShortcut);
...
msn_chat_send(PurpleConnection *gc, int id, const char *message, PurpleMessageFlags flags)
{
	...
	char *msgformat;
	char *msgtext;
/* UOL <Ticket 1187 - Send Custom Emoticon> << */
	MsnEmoticon *smile;
	GSList *smileys;
	char *emoticons = NULL;
	const char *username;
