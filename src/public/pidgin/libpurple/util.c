			...
			while (1) {
				if (badchar(*t) || badentity(t)) {
					...
					/*g_string_append_printf(ret, "<A HREF=\"%s\">%s</A>",
							tmpurlbuf, url_buf);*/
					// Force open url with anchor "#" in a new browser window
					// http://www.ibmorumbi.org.br/loja/index.asp?secao=1&categoria=8&subcategoria=0&id=9#detalhes
					if (g_strrstr(tmpurlbuf, "#") != NULL)
					{
						g_string_append_printf(ret, "<A HREF=\"%s\" target=\"_blank\">%s</A>",
								tmpurlbuf, url_buf);
					}
					else
					{
			...
			if (c[4] != '.') {
				...
				while (1) {
					if (badchar(*t) || badentity(t)) {
						...
						{
							g_string_append_printf(ret,
									"<A HREF=\"http://%s\" target=\"_blank\">%s</A>", tmpurlbuf,
									url_buf);
						}
						else
						{
